...
//
#ifndef RPRUSD_GENERATED_MESHSETTINGSAPI_H
#define RPRUSD_GENERATED_MESHSETTINGSAPI_H

/// \file rprUsd/meshSettingsAPI.h
...
/// \class RprUsdMeshSettingsAPI
///
///
class RprUsdMeshSettingsAPI : public UsdAPISchemaBase
{
public:
    ...
    /// Construct a RprUsdMeshSettingsAPI on UsdPrim \p prim .
    /// Equivalent to RprUsdMeshSettingsAPI::Get(prim.GetStage(), prim.GetPath())
    /// for a \em valid \p prim, but will not immediately throw an error for
    /// an invalid \p prim
    explicit RprUsdMeshSettingsAPI(const UsdPrim& prim=UsdPrim())
        : UsdAPISchemaBase(prim)
    {
    }

    /// Construct a RprUsdMeshSettingsAPI on the prim held by \p schemaObj .
    /// Should be preferred over RprUsdMeshSettingsAPI(schemaObj.GetPrim()),
    /// as it preserves SchemaBase state.
    explicit RprUsdMeshSettingsAPI(const UsdSchemaBase& schemaObj)
    ...
    /// Return a RprUsdMeshSettingsAPI holding the prim adhering to this
    /// schema at \p path on \p stage.  If no prim exists at \p path on
    /// \p stage, or if the prim at that path does not adhere to this schema,
    /// return an invalid schema object.  This is shorthand for the following:
    ///
    /// \code
    /// RprUsdMeshSettingsAPI(stage->GetPrimAtPath(path));
    /// \endcode
    ///
    RPRUSD_API
    static RprUsdMeshSettingsAPI
    ...
    /// This information is stored by adding "RprMeshSettingsAPI" to the 
    /// token-valued, listOp metadata \em apiSchemas on the prim.
    /// 
    /// \return A valid RprUsdMeshSettingsAPI object is returned upon success. 
    /// An invalid (or empty) RprUsdMeshSettingsAPI object is returned upon 
...
public:
    // --------------------------------------------------------------------- //
    // OBJECTVISIBILITYLIGHT 
    // --------------------------------------------------------------------- //
    /// 
    ///
    /// | ||
    /// | -- | -- |
    /// | Declaration | `bool primvars:rpr:object:visibility:light = 1` |
    /// | C++ Type | bool |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Bool |
    RPRUSD_API
    UsdAttribute GetObjectVisibilityLightAttr() const;

    /// See GetObjectVisibilityLightAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    RPRUSD_API
    UsdAttribute CreateObjectVisibilityLightAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
...
public:
    ...
    /// | ||
    /// | -- | -- |
    /// | Declaration | `uniform int primvars:rpr:mesh:subdivisionLevel = 0` |
    /// | C++ Type | int |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Int |
    /// | \ref SdfVariability "Variability" | SdfVariabilityUniform |
    RPRUSD_API
    UsdAttribute GetSubdivisionLevelAttr() const;

    /// See GetSubdivisionLevelAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    RPRUSD_API
    UsdAttribute CreateSubdivisionLevelAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
...
public:
    ...
    /// | ||
    /// | -- | -- |
    /// | Declaration | `uniform float primvars:rpr:mesh:subdivisionCreaseWeight = 0` |
    /// | C++ Type | float |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Float |
    /// | \ref SdfVariability "Variability" | SdfVariabilityUniform |
    RPRUSD_API
    UsdAttribute GetCreaseWeightAttr() const;

    /// See GetCreaseWeightAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    RPRUSD_API
    UsdAttribute CreateCreaseWeightAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;

public:
    // --------------------------------------------------------------------- //
    // IGNORECONTOUR 
    // --------------------------------------------------------------------- //
    /// Whether to extract contour for a mesh or not. Works with RPR2 only.
    ///
    /// | ||
    /// | -- | -- |
    /// | Declaration | `uniform int primvars:rpr:mesh:ignoreContour = 0` |
    /// | C++ Type | int |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Int |
    /// | \ref SdfVariability "Variability" | SdfVariabilityUniform |
    RPRUSD_API
    UsdAttribute GetIgnoreContourAttr() const;

    /// See GetIgnoreContourAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    RPRUSD_API
    UsdAttribute CreateIgnoreContourAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
